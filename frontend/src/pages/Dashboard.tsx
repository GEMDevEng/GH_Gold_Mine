...
export const Dashboard: React.FC = () => {
  ...
  const [highPotentialRepos, setHighPotentialRepos] = useState<HighPotentialRepo[]>([]);
        ...
        <CardContent>
          {highPotentialRepos.length > 0 ? (
            <div className="space-y-4">
              {highPotentialRepos.map((repo) => (
                <div key={repo._id} className="flex items-center justify-between p-4 border rounded-lg">
                  <div className="flex items-center space-x-4">
                    <div>
                      <h4 className="font-medium text-gray-900">{repo.name}</h4>
                      <p className="text-sm text-gray-500">
                        {repo.metrics.stars.toLocaleString()} stars • {repo.metrics.language || 'Unknown'}
                      </p>
                      {repo.description && (
